...
class Cache:
    ...
    def init_cache(self):
        ...
        try:
            c = conn.cursor()
            c.execute('CREATE TABLE ebooks (path TEXT PRIMARY KEY, file_hash TEXT NOT NULL, drmfree INT DEFAULT 0)')
    ...
    def get_ebook(self, path, file_hash):
        ...
        try:
            c = conn.cursor()
            c.execute('SELECT file_hash, drmfree FROM ebooks WHERE path = ?', (path,))
            obj = c.fetchone()
            if obj is not None:
    ...
    def set_ebook(self, path, file_hash, drmfree=True):
        conn = sqlite3.connect(self.ebook_cache_path)
        try:
            c = conn.cursor()
            ...
            if obj is not None:
                c.execute(
                    'UPDATE ebooks SET file_hash = ?, drmfree = ? WHERE path = ?',
                    (file_hash, int(drmfree), path)
                )
            else:
                c.execute(
                    'INSERT INTO ebooks VALUES (?,?,?)',
                    (path, file_hash, int(drmfree))
                )
            conn.commit()
        except Exception as e:
            raise CacheReadError(e)
